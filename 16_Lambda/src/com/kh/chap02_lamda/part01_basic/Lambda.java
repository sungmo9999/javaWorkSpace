...
public class Lambda {
	 ...
	 *  - OOP에서는 만들어야하는 기능(함수) 이 있는 경우 반드시 클래스를 먼저 만들고, 클래스 내부의
	 *    함수(메서드)를 추가해야한다
	 * 
	 * 람다식(Lambda expression)
	 *  - 자바에서 함수형 프로그래밍을 지원하기위해 JDK8버전부터 추가된 기능
	...
	public static void main(String[] args) {
		...
		/*
		 * - Lamdatest1인터페이스에 들어갈 객체는 반드시 LamdaTest1인터페이스를
		 *   상속받아 추상메서드를 구현한 클래스여야한다
		 * - 익명 내부 클래스의 문법처럼 이름없는 클래스릐 형태로 객체를 생성해야하지만 불필요한
		 *   코드작성을 줄이는 방식으로 간소화 하였다.
		 * - 함수형인터페이스는 고정적으로 1개의 투상메서드만 보관하고 있으므로 구현해야할
		 *   함수의 이름 반환형 매개변수의 자료형등은 생략가능
		 */
		LamdaTest1 lamda2 = (x, y) -> {return x+y;};
		result = lamda2.add(5, 5);
		System.out.println(result);
