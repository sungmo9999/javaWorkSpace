...
public class OuterClass3 {
	...
	public Runnable getRunnable(int num) {
		...
		/*
		 * 지역 내부 클래스
		 *  - 지역변수처럼 메서드내부에서 클래스를 정의하여 사용하는것
		 *  - 메서드 안에서만 사용가능하며, 매서드가 호출죌 때 생성된다.
		 *  - 지역 내부클래스안에서 사용되는 지역변수는 상수(final) 처럼 취급됨
		 *    (메모리효율성)
		 *    
		 * 메모리복습
		 *  - 메서드 호출시 스텍에는 메서드프레임이 쌓인다.
		 *  - 메서드 프레임 내부에는 지역변수를위한 저장공간이 할당
		 *  - 메서드 종료시에는 메서드프레임이 제거되면서 지역변수의 저장공간도 함께 소멸된다.
		 *    
		 * 2) static키워드가 붙은 변수는 static메모리 영역에 저장공간 할당
		 *    static final(상수) -> static메모리 내부의 constant pool에 저장공간이 추가.
		 *    final 키워드만 붙은변 수 -> static메모리 내부의 constant pool에 저장공간이 할당
		 */
		int local = 1;//지역변수
		//지역변수는 지역 내부에서 사용되는경우 final예약어가 붙은 것처럼 취급된다
		//이유는 stack메모리에서 지역변수의 저장공간이 소멸된후, 참조를 유지하기 위해.
