...
public class Run {
	public static void main(String[] args) {
		/*
		 * TCP
		 *  - 서버와 클라이어트간의 연결지향형 통신 방법
		 *  - 서버와 클라이어트 간의 연결이 되어 있어야 데이터를 교환한다
		 *  - 신뢰성있고 안전한 데이터 전달이 가능
		 *  - 클라이언트와 서버간의 연결이 되어 있으므로"실시간"으로 데이터 송수신이 가능
		 *  
		 *  
		 *  Socket
		 *   - 클라이언트와 서버간의 실시간 통신이 사용되는 객체
		 *   - 소캣이 소유한 ip번호와 port번호를 통해 소켓을 식별한다
		 *   
		 *   SeverSocket
		 *   - 서버에서 생성하는 소캣으로 클라이언트의 연결요청을 대기한다.
		 *   - 연결요청이 들어오면 Socket을 생성하여 클라이언트에게 전달
		 *   
		 *   
		 *   ClientSocket(Socket)
		 *   - 서버와 통신에 사용되는 클라이언트 소켓
		 *   - 서버의ip주소 포트번호를 통해 서버에게 연결 요청을 보낸다.
		 */
		...
		//2) 서버소켓 객체 생성
		//현재 pc의 ip 주소ㅎ와 포트 번호를 가진 소캣 생성
